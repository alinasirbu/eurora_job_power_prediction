...
#import jobs
bq --project_id [project_name] load --max_bad_records=2 --source_format=CSV eurora2.intermediate gs://[bucket]/jobs.csv.gz id:integer,job_id:string,job_name:string,queue:string,submit:timestamp,start:timestamp,end:string,user:string,nodes:integer,cpus:integer,mem:integer,wtime:string,deleted:string,dependency:string,exit_status:string,pbs_exit_code:string

#transform the string end date to timestamp end date
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.jobs "select id,job_id,job_name,queue,submit,start,if(end='0000-00-00 00:00:00',null,timestamp(end)) as finish,user,nodes,cpus,mem,wtime,deleted,dependency,exit_status,pbs_exit_code from eurora2.intermediate"

#job_nodes
bq --project_id [project_name] load --max_bad_records=2 --source_format=CSV eurora2.job_nodes gs://[bucket]/job_nodes.csv.gz id:integer,node_id:integer,job_id:string,ncpus:integer,ngpus:integer,nmics:integer,mem:integer


#power
bq --project_id [project_name] load --max_bad_records=2 --source_format=CSV eurora2.power gs://[bucket]/power.csv.gz time:timestamp,power:float


#cpu measurements
bq --project_id [project_name] load --max_bad_records=64 --source_format=CSV eurora2.cpu gs://[bucket]/cpu*.csv.gz node:integer,cpu_id:integer,time:timestamp,power:float


#gpu measurements
bq --project_id [project_name] load --max_bad_records=64 --source_format=CSV eurora2.gpu gs://[bucket]/gpu*.csv.gz node:integer,gpu_id:integer,time:timestamp,power:float


#mic measurements
bq --project_id [project_name] load --max_bad_records=64 --source_format=CSV eurora2.mic gs://[bucket]/mic*.csv.gz node:integer,mic_id:integer,time:timestamp,power:float

##################
##################
######extract features and compute job power

#extract valid power (unique)
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.power_unique "SELECT time, power*-1 as power FROM eurora2.power group by time,power"

#create time points (1st april 2014 to 31st july 2015)
 bq --project_id [project_name]  query --allow_large_results --destination_table=eurora2.aux "select time, ROW_NUMBER() OVER() from  eurora2.cpu limit 140256"

 bq --project_id [project_name]  query --allow_large_results --destination_table=eurora2.times "select date_add(timestamp('2014-04-01 00:00:00 UTC'),5*(f0_),'MINUTE') as time from  eurora2.aux"
...
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.job_nodes_complete "SELECT t1.node_id as node_id, t1.job_id as job_id, t1.ncpus as ncpus, t1.ngpus as ngpus, t1.nmics as nmics, t1.mem as mem, t2.job_name as job_name, t2.start as start,min(t2.finish) as finish, t2.user as user, min(t2.wtime) as wtime, min(t2.exit_status) as exit_status FROM eurora2.job_nodes t1 inner join each eurora2.jobs t2 on t1.job_id=t2.job_id group by node_id,job_id,ncpus,ngpus,nmics,mem,job_name,start,user"


#some null still remaining
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.job_nodes_complete_corrected "SELECT t1.node_id as node_id, t1.job_id as job_id, t1.ncpus as ncpus, t1.ngpus as ngpus, t1.nmics as nmics, t1.mem as mem, t1.job_name as job_name, t1.start as start, if(t1.finish is null,if(t1.exit_status='RUN',date_add(t1.start,if(t1.wtime='NA',0,60*INTEGER(LEFT(t1.wtime,2))+INTEGER(RIGHT(t1.wtime,2))),'MINUTE'),t1.start),t1.finish) as finish, t1.user as user FROM eurora2.job_nodes_complete t1"


#find component count - this one times out
bq --project_id [project_name]  query --dry_run --replace --allow_large_results --destination_table=eurora2.nodes_components_used "SELECT t1.time as time, t2.node_id as node_id, sum(t2.ncpus) as ncpus, sum(t2.ngpus) as ngpus, sum(nmics) as nmics FROM  eurora2.job_nodes_complete_corrected t2 cross join (select time from eurora2.power_unique where minute(t1.time)%5=0) t1 where t1.time between t2.start and date_add(t2.finish,-1,'second') or (t1.time>=t2.start and t2.finish is null) group by time, node_id "


#find component count - month by month to avoid timeout
...
bq --project_id [project_name]  query  --replace --allow_large_results --destination_table=eurora2.nodes_components_used "SELECT * FROM  $tables" &

#in this table some nodes have over 16 cores /2 gpus/2 mics used (only 732 instances so I cap the values to 16,2,2)
bq --project_id [project_name]  query  --replace --allow_large_results --destination_table=eurora2.nodes_components_used_capped "SELECT time, node_id, if(ncpus>16,16,ncpus) as ncpus, if(ngpus>2,2,ngpus) as ngpus, if(nmics>2,2,nmics) as nmics FROM  eurora2.nodes_components_used" &


##################
######for each time moment, find, for each node, total power for cores, gpus, mics 

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_cpu "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as cpu_power FROM  (select * from eurora2.cpu where (second(time) between 56 and 59 or second(time)=0) and minute(time)%5 in (0,4) and power<1000) t2 cross join eurora2.times t1 where (TIMESTAMP_TO_SEC(t1.time)-TIMESTAMP_TO_SEC(t2.time)) between 0 and 4 group by time, node_id "

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_cpu_after "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as cpu_power FROM  (select * from eurora2.cpu where (second(time) between 1 and 5) and minute(time)%5=0 and power<1000) t2 cross join eurora2.times t1 where (TIMESTAMP_TO_SEC(t2.time)-TIMESTAMP_TO_SEC(t1.time)) between 0 and 4 group by time, node_id "

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_gpu "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as gpu_power FROM  (select * from eurora2.gpu where (second(time) between 55 and 59 or second(time)=0) and minute(time)%5 in (0,4)) t2 cross join eurora2.times t1 where(TIMESTAMP_TO_SEC(t1.time)-TIMESTAMP_TO_SEC(t2.time)) between 0 and 4 group by time, node_id "

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_gpu_after "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as gpu_power FROM  (select * from eurora2.gpu where (second(time) between 1 and 5) and minute(time)%5=0) t2 cross join eurora2.times t1 where(TIMESTAMP_TO_SEC(t2.time)-TIMESTAMP_TO_SEC(t1.time)) between 0 and 4 group by time, node_id "

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_mic "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as mic_power FROM  (select * from eurora2.mic where (second(time) between 55 and 59 or second(time)=0) and minute(time)%5 in (0,4)) t2 cross join eurora2.times t1 where (TIMESTAMP_TO_SEC(t1.time)-TIMESTAMP_TO_SEC(t2.time)) between 0 and 4 group by time, node_id "

bq --project_id [project_name]  query --replace  --allow_large_results --destination_table=eurora2.nodes_total_power_mic_after "SELECT t1.time as time, t2.node as node_id, avg(t2.power)*2 as mic_power FROM  (select * from eurora2.mic where (second(time) between 1 and 5) and minute(time)%5=0) t2 cross join eurora2.times t1 where (TIMESTAMP_TO_SEC(t2.time)-TIMESTAMP_TO_SEC(t1.time)) between 0 and 4 group by time, node_id "
...
##################
##################
######correct data - many measurements can be missing for various reasons. Where possible we try to correct
...
##################
##################
######user-level data  - power usage by job

users=$(<users.csv) #file with all user names from the system
...
#download user data
bq --project_id [project_name] extract eurora2.${user}_jobs gs://[bucket]/${user}_jobs.csv
gsutil cp gs://[bucket]/${user}_jobs.csv ./${user}_jobs.csv
gzip ${user}_jobs.csv
...
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.jobs_corrected "SELECT job_id, job_name,start,finish,user FROM [eurora2.job_nodes_complete_corrected] group by job_id, job_name,start,finish,user"

bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.users_oct "SELECT left(user,8) as user FROM eurora2.jobs_corrected where month(start)=10 or month(finish)=10 group by user "


##################
##################
######averages for Enhanced average model
...
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.all_user_jobs "SELECT * FROM $tables" 	 


bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.avg_user_oct "SELECT user,avg(cpu_low_power/cores_low), avg(cpu_med_power/cores_med), avg(cpu_high_power/cores_high),avg(gpu_power/ngpus), avg(mic_power/nmics) FROM eurora2.all_user_jobs where month(time)<10 and year(time)=2014 group by user" 	 

bq --project_id [project_name] extract eurora2.avg_user_oct gs://[bucket]/avg_user_10.csv
gsutil cp gs://[bucket]/avg_user_10.csv ./avg_user_10.csv



##################
##################
###### Analysis of power fluctuations
###power vs load for all cpus and gpus

##download core info from Eurora database
...
#create bigquery tables - one for each node
for cpu in ${cpus[*]};
do
echo $cpu
bq --project_id [project_name] load --max_bad_records=64 --source_format=CSV eurora2.cores0$cpu gs://[bucket]/cores0$cpu.csv.gz core_id:integer,load_core:float,time:timestamp	&
sleep 10
done
...
do
echo $cpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load_cpu0_$cpu "SELECT t1.time as time, avg(t2.load_core) as load, t1.power as power from eurora2.cpu t1 inner join each eurora2.cores0$cpu t2 on t1.time=t2.time where t1.node=$cpu and t1.cpu_id=0 and t2.core_id<8 group by time, power" &
sleep 20
done
...
do
echo $cpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load_cpu1_$cpu "SELECT t1.time as time, avg(t2.load_core) as load, t1.power as power from eurora2.cpu t1 inner join each eurora2.cores0$cpu t2 on t1.time=t2.time where t1.node=$cpu and t1.cpu_id=1 and t2.core_id>7 group by time, power" &
sleep 10
done
...
do
echo $cpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load_cpu0_$cpu "SELECT t1.time as time, avg(t2.load_core) as load, t1.power as power from eurora2.cpu t1 inner join each eurora2.cores0$cpu t2 on t1.time=t2.timestamp where t1.node=$cpu and t1.cpu_id=0 and t2.core_id<8 group by time, power" &
sleep 20
done
...
do
echo $cpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load_cpu1_$cpu "SELECT t1.time as time, avg(t2.load_core) as load, t1.power as power from eurora2.cpu t1 inner join each eurora2.cores0$cpu t2 on t1.time=t2.timestamp where t1.node=$cpu and t1.cpu_id=1 and t2.core_id>7 group by time, power"
sleep 20
done
...
do
echo $cpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load0_avg_cpu$cpu "SELECT integer(load) as load, count(*) as count, avg(power) as avg, stddev(power) as std,  stddev(power)/avg(power) as cv  from eurora2.load_cpu0_$cpu where power<1000 group by load"&
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load1_avg_cpu$cpu "SELECT integer(load) as ld, count(*) as count, avg(power) as avg, stddev(power) as std,  stddev(power)/avg(power) as cv  from eurora2.load_cpu1_$cpu where power<1000 group by ld"&
sleep 2
done
...
do
echo $cpu
bq --project_id [project_name] extract eurora2.load0_avg_cpu$cpu gs://[bucket]/load0_avg_cpu${cpu}.csv
gsutil cp gs://[bucket]/load0_avg_cpu${cpu}.csv ./load0_avg_cpu${cpu}.csv
done

for cpu in ${cpus[*]};
do
echo $cpu
bq --project_id [project_name] extract eurora2.load1_avg_cpu$cpu gs://[bucket]/load1_avg_cpu${cpu}.csv
gsutil cp gs://[bucket]/load1_avg_cpu${cpu}.csv ./load1_avg_cpu${cpu}.csv
done
...
do
echo $gpu
bq --project_id [project_name] load --max_bad_records=64 --source_format=CSV eurora2.gpu${gpu}full gs://[bucket]/gpu${gpu}full.csv.gz gpu_id:integer,time:timestamp,pow:float,gpu_load:integer &
sleep 5
done
...
do
echo $gpu
bq --project_id [project_name]  query --replace --allow_large_results --destination_table=eurora2.load_avg_gpu$gpu "SELECT gpu_id, integer(gpu_load) as load, count(*) as count, avg(pow) as avg, stddev(pow) as std,  stddev(pow)/avg(pow) as cv  from eurora2.gpu${gpu}full group by load, gpu_id" &
sleep 5
done
...
do
echo $gpu
bq --project_id [project_name] extract eurora2.load_avg_gpu$gpu gs://[bucket]/load_avg_gpu${gpu}.csv
gsutil cp gs://[bucket]/load_avg_gpu${gpu}.csv ./load_avg_gpu${gpu}.csv
done
